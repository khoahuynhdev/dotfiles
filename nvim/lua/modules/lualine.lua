local function lsp_message()
    if vim.tbl_count(vim.lsp.buf_get_clients()) < 1 then return '' end
...
local function gps_status()
    local gps = require'utils'.prequire('SmiteshP/nvim-navic')
    if not gps or not gps.is_available() then return '' end
...
require'lualine'.setup {
    ...
    sections = {
        ...
        lualine_b = {'filename'},
        lualine_c = {gps_status},
        lualine_x = {
            'g:metals_status', lsp_message, attached_lsp, {
                'diagnostics',
                sources = {'nvim_lsp'},
