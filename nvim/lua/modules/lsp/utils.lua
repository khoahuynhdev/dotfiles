...
function M.auto_formatting(client, bufnr)
    ...
    if client.supports_method(fmt_method) then
        local formatting_ag = vim.api.nvim_create_augroup("LspFormat",
                                                          {clear = true})
        vim.api.nvim_create_autocmd("BufWritePre", {
            ...
            callback = function(...)
                ...
                local result, err = client.request_sync(fmt_method, params, 750,
                                                        bufnr)
...
function M.auto_codelenses(client, bufnr)
    ...
    if client.supports_method(codelens_method) then
        ...
        local codelens_ag = vim.api.nvim_create_augroup("LspCodeLens",
                                                        {clear = true})
        vim.api.nvim_create_autocmd({"BufEnter", "CursorHold", "InsertLeave"}, {
            group = codelens_ag,
            buffer = bufnr,
            callback = function(params) vim.lsp.codelens.refresh() end
        })
    end
end
...
function M.auto_codeaction(client, bufnr)
    ...
    if client.supports_method(codeaction_method) then
        ...
        local codeaction_ag = vim.api.nvim_create_augroup("LspCodeAction",
                                                          {clear = true})
        vim.api.nvim_create_autocmd({"CursorHold", "CursorHoldI"}, {
            group = codeaction_ag,
            buffer = bufnr,
            callback = function(params) lightbulb.update_lightbulb() end
        })
    end
end
